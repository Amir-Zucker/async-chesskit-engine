...
@Suite("Engine response parser tests")
struct EngineResponseParserTests {
    ...
    @Test func parseUciok() {
        let input = "uciok"
        #expect(EngineResponseParser.parse(response: input) == .uciok)
        #expect(EngineResponse(rawValue: input) == .uciok)
    }

    @Test func parseReadyok() {
        let input = "readyok"
        #expect(EngineResponseParser.parse(response: input) == .readyok)
        #expect(EngineResponse(rawValue: input) == .readyok)
    }
